...
function patchBulk(service, data, params) {
  ...
  const bodyReducer = mapped => (result, item) => {
    const _id = get(item[service.meta], '_id') || get(item, '_id');
    const parent = get(item[service.meta], '_parent') || get(item, '_parent');
    const routing = get(item[service.meta], '_routing') || get(item, '_routing');
